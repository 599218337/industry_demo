...
function executeDerivedCommand(command, nameDerived, commandName, scene, context, passState) {
  ...
  if (!Cesium.defined(derivedCommands)) {
    return
  }

  if (frameState.useLogDepth && Cesium.defined(derivedCommands.logDepth)) {
    command = derivedCommands.logDepth.command
  }

  derivedCommands = command.derivedCommands
  if (Cesium.defined(derivedCommands[nameDerived])) {
...
export function executeDerivedCommandList(context, targetFramebuffer, passState, nameDerived, commandName, filter) {
  scratchPerspectiveFrustum = scratchPerspectiveFrustum || new Cesium.PerspectiveFrustum()
  scratchPerspectiveOffCenterFrustum = scratchPerspectiveOffCenterFrustum || new Cesium.PerspectiveOffCenterFrustum()
  scratchOrthographicFrustum = scratchOrthographicFrustum || new Cesium.OrthographicFrustum()
  scratchOrthographicOffCenterFrustum = scratchOrthographicOffCenterFrustum || new Cesium.OrthographicOffCenterFrustum()
  ...
  for (var i = 0; i < numFrustums; ++i) {
    ...
    us.updatePass((Cesium as any).Pass.GLOBE)
    commands = frustumCommands.commands[(Cesium as any).Pass.GLOBE]
    length = frustumCommands.indices[(Cesium as any).Pass.GLOBE]

    if (globeTranslucent) {
      globeTranslucencyState.executeGlobeCommands(frustumCommands, executeCommand, undefined, scene, passState)
    } else {
      for (j = 0; j < length; ++j) {
    ...
    us.updatePass((Cesium as any).Pass.CESIUM_3D_TILE)
    commands = frustumCommands.commands[(Cesium as any).Pass.CESIUM_3D_TILE]
    length = frustumCommands.indices[(Cesium as any).Pass.CESIUM_3D_TILE]
    for (j = 0; j < length; ++j) {
      executeCommand(commands[j], scene, context, passState)
    }

    us.updatePass((Cesium as any).Pass.OPAQUE)
    commands = frustumCommands.commands[(Cesium as any).Pass.OPAQUE]
    length = frustumCommands.indices[(Cesium as any).Pass.OPAQUE]
    for (j = 0; j < length; ++j) {
      executeCommand(commands[j], scene, context, passState)
    }

    us.updatePass((Cesium as any).Pass.TRANSLUCENT)
    commands = frustumCommands.commands[(Cesium as any).Pass.TRANSLUCENT]
    length = frustumCommands.indices[(Cesium as any).Pass.TRANSLUCENT]
    for (j = 0; j < length; ++j) {
      executeCommand(commands[j], scene, context, passState)
    }
...
export default function CesiumRenderPass(options) {
  ...
  let {
    ...
    stage = null,
    useHdr,
    clearCommand = new (Cesium as any).ClearCommand({
  ...
  if (overrideViewport) {
    Cesium.BoundingRectangle.clone(overrideViewport, viewport)
  }
  ...
  function getShaderProgram(context, shaderProgram, pickId, pickIdQualifier) {
    ...
    if (!Cesium.defined(shader)) {
      ...
      // webgl1
      // var is3dtiles =
      //   /texture2D\s?\(\s?tile_pickTexture\s?,\s?tile_featureSt\s?\)/.test(
      //     pickId
      //   );
      // webgl1
      ...
      // webgl1
      // if (
      //   /attribute\s?vec3\s?normal\s?;/.test(vsStr) ||
      //   /\n\s?vec3\s?normal\s?;/.test(vsStr)
      // ) {
      //   hasNormal = true;
      ...
      if (fragmentShader) {
        ...
        let newMain =
          ...
          (Cesium as any).ShaderSource.replaceMain(fragmentShader, mainName) +
          'void main() \n' +
          '{ \n' +
        ...
        for (var i = 0; i < length; ++i) {
          newSources[i] = (Cesium as any).ShaderSource.replaceMain(sources[i], nonMainName)
        }
        ...
        fs = new (Cesium as any).ShaderSource({
          sources: newSources,
          defines: fs.defines
        })
      }
      ...
      if (vertexShader || fragmentShader) {
        ...
        let newMain =
          ...
          (Cesium as any).ShaderSource.replaceMain(vertexShader || 'void main(){}', mainName) +
          'void main() \n' +
          '{ \n' +
          (shaderRedefine != 'replace' ? `    ${nonMainName}(); \n` : '') +
        ...
        vs = new (Cesium as any).ShaderSource({
          sources: newSources,
          defines: vs.defines
        })
      }
  ...
  function getRenderState(scene, renderState) {
    ...
    if (!Cesium.defined(cacheRenderState)) {
      var rs = (Cesium as any).RenderState.getState(renderState)
      if (typeof renderStateProcess == 'function') {
        renderStateProcess.call(scope, rs)
      }
      cacheRenderState = (Cesium as any).RenderState.fromCache(rs)
  ...
  function createDerivedCommand(command, scene, context) {
    ...
    if (!Cesium.defined(derivedCommands.renderPass)) {
      result = derivedCommands.renderPass = {}
    }

    var shader
    var renderState
    if (Cesium.defined(result[commandName])) {
      shader = result[commandName].shaderProgram
      renderState = result[commandName].renderState
    }

    result[commandName] = (Cesium as any).DrawCommand.shallowClone(command, result[commandName])

    if (!Cesium.defined(shader) || result.shaderProgramId !== command.shaderProgram.id) {
  ...
  function updateUniformMap(uniformMap) {
    ...
    for (var name in uniforms) {
      if (uniforms.hasOwnProperty(name)) {
        ...
        if (typeof value === 'string' || value instanceof (Cesium as any).Texture || value instanceof HTMLImageElement || value instanceof HTMLCanvasElement || value instanceof HTMLVideoElement) {
  ...
  function udpateDerivedCommands(scene) {
    ...
    for (var i = 0; i < numFrustums; ++i) {
      ...
      for (pass = 0; pass < frustumCommands.commands.length; pass++) {
        ...
        if (stage && pass == (Cesium as any).Pass.GLOBE) continue
  ...
  function updateFramebuffer(context, viewport, hdr, sceneFramebuffer) {
    ...
    colorTexture = new (Cesium as any).Texture({
      context: context,
      width: width,
      height: height,
      pixelFormat: Cesium.defaultValue(pixelFormat, Cesium.PixelFormat.RGBA),
      pixelDatatype: Cesium.defaultValue(pixelDatatype, Cesium.PixelDatatype.FLOAT),
      sampler: Cesium.defaultValue(sampler, (Cesium as any).Sampler.NEAREST)
    })
    depthTexture = new (Cesium as any).Texture({
      context: context,
      width: width,
      height: height,
      pixelFormat: Cesium.PixelFormat.DEPTH_COMPONENT,
      pixelDatatype: Cesium.PixelDatatype.UNSIGNED_SHORT,
      sampler: (Cesium as any).Sampler.NEAREST
    })
  ...
  function getPassState(view) {
    if (!overrideViewport) {
      viewport = Cesium.BoundingRectangle.clone(view.viewport, viewport)
      if (viewportScale) {
  ...
  function update(context, useLogDepth) {
    ...
    try {
      if (!vertexShader && !fragmentShader) {
        ...
        executeDerivedCommandList(context, framebuffer, passState, undefined, undefined, undefined)
      }
      //   if (stage && stage._selectedIdTexture)
        ...
        executeDerivedCommandList(context, framebuffer, passState, 'renderPass', commandName, renderType == 'all' || !stage || !stage._selectedIdTexture ? null : commandFilter)
      }
    } catch (err) {
      console.error(err)
    }
    ...
    if (uniformMap[command.pickId]) {
      ...
      for (let i = 0; i < selectedFeatures.length; i++) {
        ...
        for (let j = 0; j < pickIds.length; j++) {
          ...
          if (pickId.color == id || Cesium.Color.equals(pickId.color, id)) {
            return renderSelected
          }
        }
      }
