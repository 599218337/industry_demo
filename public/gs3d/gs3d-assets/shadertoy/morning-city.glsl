// 于2023/12/14摘自于 https://www.shadertoy.com/view/XsBSRG 的像素着色器片段
...
vec3 cars(vec2 squarer, vec2 pos, float dist, float level) {
    ...
    for(int j = 0; j < 10; j++) {
        ...
        if(fract(random * 5.0 / i) > carNumber) {
            color += car1 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(fract(i + iTime / 4.0), 0.025)));
        }

        if(fract(random * 10.0 / i) > carNumber) {
            color += car2 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(fract(i - iTime / 4.0), 0.975)));
        }
        if(color.x > 0.0)
            break;
    }
    for(int j = 0; j < 10; j++) {
        float i = 0.03 + float(j) * 0.094;
        if(fract(random * 5.0 / i) > carNumber) {
            color += car2 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(0.025, fract(i + iTime / 4.0))));
        }
        if(fract(random * 10.0 / i) > carNumber) {
            color += car1 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(0.975, fract(i - iTime / 4.0))));
        }
        if(color.x > 0.0)
            break;
    ...
    for(int j = 0; j < 10; j++) {
        ...
        if(fract(random * 100.0 / i) > carNumber) {
            color += car1 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(fract(i + iTime / 4.0), 0.045)));
        }
        if(fract(random * 1000.0 / i) > carNumber) {
            color += car2 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(fract(i - iTime / 4.0), 0.955)));
        }
        if(color.x > 0.0)
            break;
    ...
    for(int j = 0; j < 10; j++) {
        float i = 0.03 + 0.047 + float(j) * 0.094;
        if(fract(random * 100.0 / i) > carNumber) {
            color += car2 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(0.045, fract(i + iTime / 4.0))));
        }
        if(fract(random * 1000.0 / i) > carNumber) {
            color += car1 * carInten * smoothstep(carRadis, 0.0, length(pos - vec2(0.955, fract(i - iTime / 4.0))));
        }
        if(color.x > 0.0)
            break;
