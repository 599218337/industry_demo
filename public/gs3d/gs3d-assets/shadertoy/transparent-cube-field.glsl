// 于2023/12/14摘自于 https://www.shadertoy.com/view/ll2SRy 的像素着色器片段
...
void mainImage(out vec4 fragColor, vec2 fragCoord) {
    ...
    col = mix(col, pow(col.x * vec3(1.5, 1, 1), vec3(1, 2.5, 12)), dot(sin(rd.yzx * 8. + sin(rd.zxy * 8.)), vec3(.1666)) + .4);
    ...
    col = mix(col, vec3(col.x * col.x * .85, col.x, col.x * col.x * .3), dot(sin(rd.yzx * 4. + sin(rd.zxy * 4.)), vec3(.1666)) + .25);
