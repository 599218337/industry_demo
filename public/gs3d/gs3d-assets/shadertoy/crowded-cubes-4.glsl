// 于2023/12/14摘自于 https://www.shadertoy.com/view/MlBSRy 的像素着色器片段
...
void mainImage(out vec4 f, vec2 w) {
    ...
    vec4 p = vec4(w, 0, 1) / iResolution.yyxy - .5, d;
    p.x -= .4; // init ray 
    r(p.xz, .13);
    r(p.yz, .2);
    r(p.xy, .1);   // camera rotations
    ...
    for(float i = 1.; i > 0.; i -= .01) {
        ...
        n = fract(u.x + u.y + u.z);
        if(n < .25)
            x3 = max(ta.x - r2, M.y - r); // growing plate
        else if(n < .5)
            x3 = max(ta.x - r, M.y - r2); // growing bar
