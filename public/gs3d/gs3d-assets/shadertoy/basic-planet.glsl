// 于2023/12/14摘自于 https://www.shadertoy.com/view/7dcBDs 的像素着色器片段
...
float noise(vec3 v3) {
    ...
    return 2.0 * mix(mix(mix(hash(i + bin.xxx), hash(i + bin.yxx), b.x), mix(hash(i + bin.xyx), hash(i + bin.yyx), b.x), b.y), mix(mix(hash(i + bin.xxy), hash(i + bin.yxy), b.x), mix(hash(i + bin.xyy), hash(i + bin.yyy), b.x), b.y), b.z) - 1.0;
}
