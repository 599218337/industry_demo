// 于2023/12/14摘自于 https://www.shadertoy.com/view/tdjSWR 的像素着色器片段
...
bool isOutClosed(vec1 a, vec1 s, vec1 e) {
    return e - s < abs(a * 2. - e - s);
}//return (a>e)&&(a<s)
bool isOutClosed(vec2 a, vec2 s, vec2 e) {
    return any(lessThan(e - s, abs(a * 2. - e - s)));
}
bool isOutClosed(vec3 a, vec3 s, vec3 e) {
    return any(lessThan(e - s, abs(a * 2. - e - s)));
}
bool isOutClosed(vec4 a, vec4 s, vec4 e) {
    return any(lessThan(e - s, abs(a * 2. - e - s)));
}
...
bool isInOpen(vec1 a, vec1 s, vec1 e) {
    return abs(a * 2. - e - s) < e - s;
}//return (a>s)&& (a<e) 
bool isInOpen(vec2 a, vec2 s, vec2 e) {
    return any(lessThan(abs(a * 2. - e - s), e - s));
}
bool isInOpen(vec3 a, vec3 s, vec3 e) {
    return any(lessThan(abs(a * 2. - e - s), e - s));
}
bool isInOpen(vec4 a, vec4 s, vec4 e) {
    return any(lessThan(abs(a * 2. - e - s), e - s));
}
...
vec4 setg(vec2 a, vec2 b) {
    return pmpm * min(a.xxyy * pmpm, b.xxyy * pmpm);
}
vec3 polygon(
    vec2 c,
    vec4 g,
    ...
    if(any(lessThan(vec4(c.xy, g.yw), vec4(g.xz, c.xy))) || dot(d, u) < 0.)
        return vec3(101, 0, 0);
...
vec3 polygonXY(
    ...
    vec2 c = u.xy - d.xy * (u.z - z) / d.z;
    return polygon(c, setg(a, b), d, vec3(c, z) - u);
}
vec3 polygonYZ(
    float x,
    vec2 a,
    ...
    vec2 c = u.yz - d.yz * (u.x - x) / d.x;
    return polygon(c, setg(a, b), d, vec3(x, c) - u);
}
vec3 polygonXZ(
    float y,
    vec2 a,
    ...
    vec2 c = u.xz - d.xz * (u.y - y) / d.y;
    return polygon(c, setg(a, b), d, vec3(c.x, y, c.y) - u);
}
...
vec3 textureWall(
    ...
    vec3 windowColor = (-0.4 + randB * 0.8) * vec3(0.3, 0.3, 0.0) + (-0.4 + fract(randB * 10.0) * 0.8) * vec3(0.0, 0.0, 0.3) + (-0.4 + fract(randB * 10000.) * 0.8) * vec3(0.3, 0.0, 0.0);
    float floorFactor = 1.;
    vec2 windowSize = vec2(0.65, 0.35);
    vec3 wallColor = s * (0.3 + 1.4 * fract(randB * 100.)) * vec3(0.1, 0.1, 0.1) + (-0.7 + 1.4 * fract(randB * 1000.)) * vec3(0.02, 0., 0.);
    wallColor *= 1.3;
    vec3 color = vec3(0);
    ...
    float w = wsize * 2.;
    bvec3 bo = bvec3(isOutOpen(pc.x, w * n.y, w + w * n.y) || isOutOpen(maxPos.x, .5, .6), isOutOpen(pc.xx, w * m, w + w * m) || isOutOpen(maxPos.x, .6, .7), isOutOpen(pc.xxx, w * n, w + w * n) || maxPos.x > .7);
    bnot(bo);
    if(any(bo))
        return (.9 + .2 * noise(pos)) * conturColor;
    ...
    if((random < 0.05 * (3.5 - 2.5 * floorFactor)) || (windows > 0.65)) {
        if(winPos.y < 0.5)
            windowColor *= (1.0 - 0.4 * fract(random * 100.));
...
void carloop(
    ...
    for(float j = 0.; j < 10.; j++) {
        ...
        if(e.z > 0.)
            a = a.yx;
        ;
        if(fract(n * 5. / i) > carNumber)
            c += car1 * smoothstep(r, 0., length(u - a));
        a = vec2(fract(i - iTime / 4.), e.y);
        if(e.z > 0.)
            a = a.yx;
        ;
        if(fract(n * 10. / i) > carNumber)
            c += car2 * smoothstep(r, 0., length(u - a));
        if(c.x > 0.)
            break;
    }
}
...
#ifdef CARS
    ;
    if(min(pos.x, pos.y) < 0.07 || max(pos.x, pos.y) > 0.93)
        color += cars(squarer, pos, dist, 0.);
#endif
    ;
    return color;
}
...
void mainImage(
    ...
    if(iMouse.z > 0.)
        camTarget.xy = camPos.xy - cs(6.2 * iMouse.x / iResolution.x);
  ...
  //FoV like its the 80s, makes sense here to debug the traverser
    ...
    float angle = .03 * pow(abs(acos(d.x)), 4.0);//angle=min(0.0,angle)
    ;
    vec3 color = vec3(0.0);
    ...
    for(int i = 1; i < I_MAX; i++ //2d rectangle traverse loop
        ...
        if(crossSky == squarer && crossSky != floor(camPos.xy)) {
            color += vec3(vec2(.5, .15) * abs(angle) * exp(-d.z * d.z * 30.), .2);
            break;
        };
        float random = rand(squarer), t, height = 0.;
        float quartalR = rand(floor(squarer / 10.));
        if(floor(squarer / 10.) == vec2(0))
            quartalR = .399;
        if(quartalR < .4) {
            height = -.15 + .4 * random + smoothstep(12., 7., length(fract(squarer / 10.) * 10. - vec2(5))) * .8 * random + .9 * smoothstep(10., 0., length(fract(squarer / 10.) * 10. - vec2(5)));
            height *= quartalR / .4;
        };
        float maxJ = 2.;
        float roof = 1.;
        ...
        if(camPos.z + d.z * (length(camPos.xy - square) + 0.71 - sign(d.z) * 0.71) / length(d.xy) < maxheight) {
            ...
            for(int nf = 1; nf < 8; nf++) {
                ...
                if(j > maxJ)
                    break;
                prevZZ = zz;
                zz = 1.5 * pow(j, 0.3) * height;//prevZZ=zz-0.8
                ;
                ...
                if(randomF < .25) {
                    v1 = vec2(fract(random * 1000.), fract(random * 100.));
                };
                if(randomF > .25 && randomF < .5) {
                    v1 = vec2(fract(random * 100.), 0.);
                    v2 = vec2(0.0, fract(random * 1000.));
                };
                if(randomF > .5 && randomF < .75) {
                    v2 = vec2(fract(random * 1000.), fract(random * 100.));
                };
                if(randomF > .75) {
                    v1 = vec2(0., fract(random * 1000.));
                    v2 = vec2(fract(random * 100.), 0.);
                };
                if(d.y < 0.0) {
                    float y = v1.y;
                    v1.y = v2.y;
                ...
                if(pxy.x < mind) {
                    ...
                    pol = pxy;
                    k = 1;
                    maxPos = vec2(abs(a.x - b.x), abs(a.y - b.y));
                };
                ...
                if(pyz.x < mind) {
                    ...
                    pol = pyz;
                    k = 2;
                    maxPos = vec2(abs(a.y - b.y), zz - prevZZ);
                };
                ...
                if(pxz.x < mind) {
                    ...
                    pol = pxz;
                    k = 3;
                    maxPos = vec2(abs(a.x - b.x), zz - prevZZ);
                }
            };
            if((mind < 100.) && (k == 1)) {
        ...
        if((square.x + sign(d.x) - camPos.x) / d.x < (square.y + sign(d.y) - camPos.y) / d.y) {
            square.x += sign(d.x) * 1.0;
        } else
            square.y += sign(d.y) * 1.;
        if(i == I_MAX - 1 && d.z > -0.1)
            color += vec3(vec2(0.5, 0.15) * abs(angle) * exp(-d.z * d.z * 30.0), 0.2);
    };
