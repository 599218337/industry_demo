// 于2023/12/14摘自于 https://www.shadertoy.com/view/ldfGDN 的像素着色器片段
...
float hash(in float x) {
    return fract(sin(x * .0007) * 29835.24389);
}
float hash(in vec2 x) {
    return hash(dot(x, vec2(23.17, 17.23)));
}
float hash(in vec3 x) {
    return hash(dot(x, vec3(23.17, 17.23, 42.5)));
}
float noise_value(in vec2 p) {
    vec2 e = vec2(1., 0.), F = floor(p), f = fract(p), k = (3. - 2. * f) * f * f;
    return mix(mix(hash(F), hash(F + e.xy), k.x), mix(hash(F + e.yx), hash(F + e.xx), k.x), k.y);
}
float noise_value(in vec3 p) {
    vec2 e = vec2(1., 0.);
    vec3 F = floor(p), f = fract(p), k = (3. - 2. * f) * f * f;
    return mix(mix(mix(hash(F), hash(F + e.xyy), k.x), mix(hash(F + e.yxy), hash(F + e.xxy), k.x), k.y), mix(mix(hash(F + e.yyx), hash(F + e.xyx), k.x), mix(hash(F + e.yxx), hash(F + e.xxx), k.x), k.y), k.z);
}
...
float world(in vec3 at, in vec3 dir) {
    ...
    if(at.y > CLOUDS_HEIGHT_MIN) {
        vec3 cs = at * .001 - t * .3;
        float k = sin((at.y - CLOUDS_HEIGHT_MIN) / (CLOUDS_HEIGHT_MAX - CLOUDS_HEIGHT_MIN) * c_pi), v = cloud_noise(cs) * k, V = max(0., v * v - .13);
        if(V > .0) {
...
vec3 normal(in vec3 at) {
    vec2 e = vec2(.1, .0);
    return normalize(vec3(world(at + e.xyy, vec3(0.)) - world(at, vec3(0.)), world(at + e.yxy, vec3(0.)) - world(at, vec3(0.)), world(at + e.yyx, vec3(0.)) - world(at, vec3(0.))));
}
...
vec3 light(in vec3 at, in vec3 from, in light_dir_t l) {
    ...
    return l.color * (cmat.diffuse * max(0., dot(n, l.dir)) + pow(max(0., dot(n, h)), cmat.specular) * (cmat.specular + 8.) / (8. * c_pi));
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(iMouse.z <= 0.) {
        ...
        mat3 r = mat3(c, 0., -s, 0., 1., 0., s, 0., c);
        ray = lookAtDir(normalize(vec3(uv, 2.)), vec3(1500. * t, 0., 0.) + r * vec3(6000., 100. + 6000. * (1. - mp.y), 6000.), vec3(1500. * t, 0., 0.));
    }
