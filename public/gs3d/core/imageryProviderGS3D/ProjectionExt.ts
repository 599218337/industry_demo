...
let _MapProjectionType = Object.freeze(MapProjectionType)
...
function SerializedMapProjection(mapProjectionType, json) {
  ...
  // Cesium.Check.typeOf.number('mapProjectionType', mapProjectionType)
  // Cesium.Check.defined('json', json)
  //>>includeEnd('debug');
  function getFailedTypeErrorMessage(actual, expected, name) {
    return `Expected ${name} to be typeof ${expected}, actual typeof was ${actual}`
  }
  function number(name, test) {
    if (typeof test !== 'number') {
      throw new Cesium.DeveloperError(getFailedTypeErrorMessage(typeof test, 'number', name))
    }
  }
  ...
  function defined(name, test) {
    if (!Cesium.defined(test)) {
      throw new Cesium.DeveloperError(getUndefinedErrorMessage(name))
    }
  }
  number('mapProjectionType', mapProjectionType)
  defined('json', json)
...
var proj4Src: any = createCommonjsModule(function (module, exports) {
  ;(function (global, factory) {
    ...
    function Projection(srsCode, callback?: Function): void {
    ...
    function transformer(from, to, coords, enforceAxis?: any) {
    ...
    function LLtoUTM(ll) {
      ...
      var UTMNorthing =
        k0 *
        (M + N * Math.tan(LatRad) * ((A * A) / 2 + ((5 - T + 9 * C + 4 * C * C) * A * A * A * A) / 24.0 + ((61 - 58 * T + T * T + 600 * C - 330 * eccPrimeSquared) * A * A * A * A * A * A) / 720.0))
      if (Lat < 0.0) {
    ...
    function UTMtoLL(utm) {
      ...
      phi1Rad =
        mu + ((3 * e1) / 2 - (27 * e1 * e1 * e1) / 32) * Math.sin(2 * mu) + ((21 * e1 * e1) / 16 - (55 * e1 * e1 * e1 * e1) / 32) * Math.sin(4 * mu) + ((151 * e1 * e1 * e1) / 96) * Math.sin(6 * mu)
    ...
    function Point(x, y?: any, z?: any): void {
            ...
            (this.k0 *
              (ml -
                ...
                ((sin_phi * delta_lon * al) / 2) *
                  (1 + (als / 12) * (5 - t + 9 * c + 4 * cs + (als / 30) * (61 + ts - 58 * t + 270 * c - 330 * t * c + (als / 56) * (1385 + 543 * ts - ts * t - 3111 * t)))))) +
          this.y0
      }
      ...
      if (!this.es) {
        ...
        if (Math.abs(phi) < HALF_PI) {
          ...
          lat =
            ...
            ((con * ds) / (1 - this.es)) *
              0.5 *
              (1 - (ds / 12) * (5 + 3 * t - 9 * c * t + c - 4 * cs - (ds / 30) * (61 + 90 * t - 252 * c * t + 45 * ts + 46 * c - (ds / 56) * (1385 + 3633 * t + 4095 * ts + 1574 * ts * t))))

          lon = adjust_lon(
            this.long0 + (d * (1 - (ds / 6) * (1 + 2 * t + c - (ds / 20) * (5 + 28 * t + 24 * ts + 8 * c * t + 6 * c - (ds / 42) * (61 + 662 * t + 1320 * ts + 720 * ts * t))))) / cos_phi
          )
        } else {
          lat = HALF_PI * sign(y)
          lon = 0
    ...
    function inverse$10(p) {
      ...
      do {
        p.y =
          2 *
          (Math.atan(Math.pow(this.k, -1 / this.alfa) * Math.pow(Math.tan(u / 2 + this.s45), 1 / this.alfa) * Math.pow((1 + this.e * Math.sin(fi1)) / (1 - this.e * Math.sin(fi1)), this.e / 2)) -
            this.s45)
    ...
    var imlfn = function (ml, e0, e1, e2, e3) {
      ...
      for (var i = 0; i < 15; i++) {
        dphi =
          (ml - (e0 * phi - e1 * Math.sin(2 * phi) + e2 * Math.sin(4 * phi) - e3 * Math.sin(6 * phi))) / (e0 - 2 * e1 * Math.cos(2 * phi) + 4 * e2 * Math.cos(4 * phi) - 6 * e3 * Math.cos(6 * phi))
        phi += dphi
        if (Math.abs(dphi) <= 0.0000000001) {
          return phi
        }
      ...
      if (this.sphere) {
        ...
        if (Math.abs(y + this.ml0) <= EPSLN) {
          ...
          for (i = MAX_ITER$2; i; --i) {
            ...
            ma = mln / this.a
            dphi =
              (al * (cl * ma + 1) - ma - 0.5 * cl * (ma * ma + bl)) / ((this.es * Math.sin(2 * phi) * (ma * ma + bl - 2 * al * ma)) / (4 * cl) + (al - ma) * (cl * mlnp - 2 / Math.sin(2 * phi)) - mlnp)
            phi -= dphi
            if (Math.abs(dphi) <= EPSLN) {
              lat = phi
              break
...
//默认坐标系定义
proj4Src.defs('EPSG:4528', '+proj=tmerc +lat_0=0 +lon_0=120 +k=1 +x_0=40500000 +y_0=0 +ellps=GRS80 +units=m +no_defs')
proj4Src.defs('EPSG:4547', '+proj=tmerc +lat_0=0 +lon_0=114 +k=1 +x_0=500000 +y_0=0 +ellps=GRS80 +units=m +no_defs')
proj4Src.defs('EPSG:4549', '+proj=tmerc +lat_0=0 +lon_0=120 +k=1 +x_0=500000 +y_0=0 +ellps=GRS80 +units=m +no_defs')
proj4Src.defs('EPSG:27700', '+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 +x_0=400000 +y_0=-100000 +ellps=airy +towgs84=446.448,-125.157,542.06,0.15,0.247,0.842,-20.489 +units=m +no_defs')

var UNBOUNDED = new Cesium.Rectangle(Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY)

type Proj4ProjectionOptions = {
  wellKnownText?: string
  wkt?: string
  wkid?: number
  heightScale?: number
  /**
   * 图层的矩形。访问切片图层时，将忽略此参数。矩形([west, south, east, north])，影像所覆盖的范围。
   *
   * west - 最西端的经度，范围为 [-180,180]。
   *
   * south - 最南端的纬度，范围为 [-90,90]。
   *
   * east - 最东端的经度，范围为 [-180,180]。
   *
   * north - 最北端的纬度，范围为 [-90,90]。
   * @defaultValue [0,0,0,0]
   * @group 3.可选参数
   * @example
   * ```ts
   * wgs84Bounds: [-650000, -150000, 1350000, 1450000]
   * wgs84Bounds:[96.799393, -43.598214999057824, 153.63925700000001, -9.2159219997013]
   * ```
   */
  wgs84Bounds?: [west: number, south: number, east: number, north: number] | Cesium.Rectangle
  /**
   * 图层的矩形。访问切片图层时，将忽略此参数。矩形([west, south, east, north])，影像所覆盖的范围。
   *
   * west - 最西端的经度，范围为 [-180,180]。
   ...
   * @example
   * ```ts
   * projectedBounds: [-650000, -150000, 1350000, 1450000]
   * projectedBounds:[96.799393, -43.598214999057824, 153.63925700000001, -9.2159219997013]
   * ```
   */
  projectedBounds?: [west: number, south: number, east: number, north: number] | Cesium.Rectangle
  /**
   ...
   * ```
   */
  ellipsoid?: [x: number, y: number, z: number] | Cesium.Ellipsoid
}

/**
 * 地图投影使用 PROJ.4 风格的知名文本。
 * 用户在使用局部区域投影时应使用 <code>options.wgs84Bounds</code> 参数，
 * 因为局部区域投影可能会在其有效边界之外产生意外结果
 * 在制图空间中。这些意外结果可能会导致Cesium中其他地方的计算错误。
 *
 * {@link Globe.cartographicLimitRectangle} 也可能有助于减少
 * 由于将坐标固定到 <code>options.wgs84Bounds</code> 而导致的视觉伪影。
 *
 * 使用 Proj4Projection 的场景将默认为 <code>MapMode2D.ROTATE</code> 而不是 <code>MapMode2D.INFINITE_SCROLL</code>。
 *
 * @alias Proj4Projection
 * @constructor
 *
 * @param {Object} options 具有以下属性的对象:
 * @param {String} options.wellKnownText 指定投影的 PROJ4 样式的已知文本。默认为 EPSG：3857（Web 墨卡托）的已知文本。
 * @param {Number} [options.heightScale=1.0] 比例以从高度（以米为单位）转换为投影的单位。
 * @param {Rectangle} [options.wgs84Bounds] 投影有效的制图边界。制图点将在投影之前固定到这些边界。
 * @param {Rectangle} [options.projectedBounds] 逆投影有效的投影边界。投影点将在取消投影之前被钳制到这些边界。
 ...
 */
export function Proj4Projection(options: Proj4ProjectionOptions) {
  options = Cesium.defaultValue(options, Object.freeze({}))

  //>>includeStart('debug', pragmas.debug);
  // Check.typeOf.string("options.wellKnownText", options.wellKnownText);
  //>>includeEnd('debug');

  let wkt = options.wellKnownText || options.wkt
  let wkid = options.wkid || 3857
  let heightScale = Cesium.defaultValue(options.heightScale, 1.0)
  let wgs84Bounds = Cesium.defaultValue(options.wgs84Bounds, Cesium.Rectangle.MAX_VALUE)
  let projectedBounds = Cesium.defaultValue(options.projectedBounds, UNBOUNDED)
  this._ellipsoid = Cesium.defaultValue(options.ellipsoid, Cesium.Ellipsoid.WGS84)
  this._projection = wkt ? proj4Src(wkt) : proj4Src(`EPSG:${wkid}`)
  this._wkt = wkt
  this._wkid = wkid
  this._heightScale = heightScale
  this._inverseHeightScale = 1.0 / heightScale
  this._wgs84Bounds = Cesium.Rectangle.clone(wgs84Bounds)
  this._projectedBounds = Cesium.Rectangle.clone(projectedBounds)
}
...
Proj4Projection.prototype.serialize = function () {
  var json = {
    ...
    ellipsoid: Cesium.Ellipsoid.pack(this.ellipsoid, []),
    wgs84Bounds: Cesium.Rectangle.pack(this.wgs84Bounds, []),
    projectedBounds: Cesium.Rectangle.pack(this.projectedBounds, []),
    heightScale: this.heightScale
  }
  return new SerializedMapProjection(_MapProjectionType.PROJ4, json)
}

/**
 * 从输入的 JSON 重构 Proj4Projection对象。
 *
 * @private
 * @param {SerializedMapProjection} serializedMapProjection 可以重建 MapProjection 的 JSON 对象。
 * @returns {Promise.<Proj4Projection>} 解析为可随时使用的 MapProjection 的 Promise，如果序列化的 MapProjection 是畸形的，则拒绝解析。
 */
Proj4Projection.deserialize = function (serializedMapProjection) {
  var json = serializedMapProjection.json
  var ellipsoid = Cesium.Ellipsoid.unpack(json.ellipsoid)
  var wgs84Bounds = Cesium.Rectangle.unpack(json.wgs84Bounds)
  var projectedBounds = Cesium.Rectangle.unpack(json.projectedBounds)
...
Proj4Projection.prototype.project = function (cartographic, result) {
  //>>includeStart('debug', pragmas.debug);
  // Cesium.Check.defined('cartographic', cartographic)
  //>>includeEnd('debug');
  function getUndefinedErrorMessage(name) {
    return `${name} is required, actual value was undefined`
  }
  ...
  defined('cartographic', cartographic)
  if (!Cesium.defined(result)) {
    result = new Cesium.Cartesian3()
  }
  var wgs84Bounds = this.wgs84Bounds
  scratchProjectionArray[0] = Cesium.Math.toDegrees(Cesium.Math.clamp(cartographic.longitude, wgs84Bounds.west, wgs84Bounds.east))
  scratchProjectionArray[1] = Cesium.Math.toDegrees(Cesium.Math.clamp(cartographic.latitude, wgs84Bounds.south, wgs84Bounds.north))
...
Proj4Projection.prototype.unproject = function (cartesian, result) {
  //>>includeStart('debug', pragmas.debug);
  // Cesium.Check.defined('cartesian', cartesian)
  //>>includeEnd('debug');
  function getUndefinedErrorMessage(name) {
    return `${name} is required, actual value was undefined`
  }
  ...
  defined('cartesian', cartesian)
  if (!Cesium.defined(result)) {
    result = new Cesium.Cartographic()
  }
  var projectedBounds = this.projectedBounds
  scratchProjectionArray[0] = Cesium.Math.clamp(cartesian.x, projectedBounds.west, projectedBounds.east)
  scratchProjectionArray[1] = Cesium.Math.clamp(cartesian.y, projectedBounds.south, projectedBounds.north)
  var unprojected = this._projection.inverse(scratchProjectionArray)
  result.longitude = Cesium.Math.toRadians(unprojected[0])
  result.latitude = Cesium.Math.toRadians(unprojected[1])
