...
export namespace heatMap {
  export const describe: string = '热力图'
  /**
  ...
  * @example
  ```ts
  //绘制热力图
  let heatmap = new gs3d.thematic.heatMap.Heatmap(viewer, {
 ...
 */
  export class Heatmap {
    ...
    constructor(viewer: any, opt: any) {
      this.viewer = viewer
      if (this.opt) {
    ...
    init() {
      ...
      for (let i = 0; i < this.hierarchy.length; i++) {
        ...
        const rete: any = this.computeRateInBound(p1)
        points.push({
          x: rete.x,
          y: rete.y,
    ...
    bindTooltip() {
      ...
      this.popupHandler || (this.popupHandler = new Cesium.ScreenSpaceEventHandler(this.viewer.scene.canvas)),
        this.popupHandler.setInputAction(function (t) {
          ...
          if (i && ((i.primitive && 'gs3d-heatmap' == i.primitive.id) || (typeof i.id == 'object' && 'gs3d-heatmap' == i.id.name))) {
      ...
      features &&
        features.forEach((item: any) => {
          if (item.geometry.type !== 'Point') {
            item.geometry = common.getCenterGeometry(item.geometry)
          }
          this.list.push({
    ...
    computeRateInBound(position: any) {
      if (!position) return
    ...
    computeBound(positions: any) {
      ...
      let lnglats: any = cartesiansToLnglats(rect, this.viewer)
      ...
      this.extentRectangle = Cesium.Rectangle.fromDegrees(minLng - diff_lng / 3, minLat - diff_lat / 3, maxLng + diff_lng / 3, maxLat + diff_lat / 3)
    ...
    createGeometry() {
      ...
      let geometry = new Cesium.Geometry({
        ...
        primitiveType: Cesium.PrimitiveType[this.primitiveType] as unknown as Cesium.PrimitiveType,
    ...
    createDom() {
      ...
      let mapDom: any = window.document.getElementById(this.viewer.container.id)
   ...
   */
  const cartesianToLnglat = function (cartesian: any, viewer: any) {
    if (!cartesian) return []
   ...
   */
  const cartesiansToLnglats = function (cartesians: any, viewer: any) {
