 ...
 * @LastEditors GS3D
 * @LastEditTime 2024-01-22 10:11:17
 * Copyright 2024
 * listeners
 */
...
export default class Cluster {
  ...
  draw(opt) {
    ...
    let dataSourcePromise = this.viewer.dataSources.add(this.dataSource)
  ...
  addData(data) {
    ...
    data.forEach(item => {
      if (item?.geometry?.coordinates?.length) {
        ...
        let center: any = geometry.type == 'Point' ? { geometry } : centroid({ type: 'Feature', geometry })
        let coordinates: [number, number] = center.geometry.coordinates
        let z = Number(properties?.height) || 0
        let param: any = {
          properties: properties,
          position: Cesium.Cartesian3.fromDegrees(...coordinates, z),
          billboard: {
    ...
    this.handler = function (clusteredEntities, cluster) {
      ...
      interval.forEach(item => {
        if (clusteredEntities.length >= item.interval[0] && clusteredEntities.length < item.interval[1]) {
          cluster.billboard.image = _this.#combineIconAndLabel(item.image, clusteredEntities.length, originImageSize || 64)
